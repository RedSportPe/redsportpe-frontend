...
- Operator (in-store POS): shops the SAME store UI as a customer, but checkout switches to "Venta en tienda" — no delivery data, optional customer name, payment Efectivo (must enter received amount ≥ total; change shown; confirming without it shakes/pulses like the admin unsaved guard) or QR confirmed instantly. registerPosSale in OrdersStore: order born method 'tienda', PAID + DELIVERED (tracking Registrado→Pagado→Empacado→Entregado all done), paymentMethod + cashReceived stored, and NO RedSport points (they belong to the buyer, not the operator).
