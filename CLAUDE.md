...
- Future contexts: promotions (public name: "Promos"), inventory, payments, shipping, notifications, marketing
...
- Data source: `public/data/products.json` via CatalogRepository (HttpClient). Will be swapped for the real API by changing only the repository.
...
- Done: catalog context complete, home sections (trends carousel, novedades, WhatsApp club, membership), cart with drawer.
- Next candidates: feature/auth (IAM, Google login), feature/promotions ("Promos" section), or starting the backend (10 bounded contexts from event storming, stack TBD).
